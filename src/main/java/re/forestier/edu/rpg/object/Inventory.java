...
public class Inventory {
    ...
    private Map<String, RPG> inventory;
    ...
    public void addObject(String name, int freeWeight) {
        ...
        RPG object = ObjectList.getObject(name);
        if (object.getWeight()<freeWeight) inventory.put(name, object);
    }

    public void addObjects(ArrayList<String> names, int maxWeight) {
        for (String name : names) addObject(name, maxWeight);
    }
    ...
    public Map<String, RPG> getInventory() {
        return new HashMap<>(inventory); 
    }

     public int getTotalWeight() {
        int totalWeight = 0;
        for (RPG object : inventory.values()) {
    ...
    public Collection<RPG> getObjects(){
